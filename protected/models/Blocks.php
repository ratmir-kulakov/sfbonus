...
class Blocks extends CActiveRecord
{
     ...
     * @return array The liast of blocks
     */
    public function getItemsByPosition($position)
    {
        $items = $this->findAllByAttributes(array('position'=>$position), 'status=:status', array(':status'=>1));
        return $items;
    }
    ...
    {
        $items = $this->findByAttributes(array('position'=>$position), 'status=:status and weight=0', array(':status'=>1));
        return $items;
    }
}